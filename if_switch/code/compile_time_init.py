...
def make_compile_inits(parse_tree):
    ...
    for array in array_value:
        if (type(array_value[array]['value']) == int):
            rhs = '{' + (str(array_value[array]['value']) + ',') * (array_value[array]['upper'] - 1) + str(
...
def find_array(i, n, l, loop_var, lower, upper, op, inc):
    ...
    if (i == n):
        return
    if (type(l[i]) == list and len(l[i]) == 3 and type(l[i][0]) == list and l[i][1] == '=' and array_value[l[i][0][0]][
        'value'] == 'garbage'):
        if (l[i][0][2] == loop_var):
            if (type(l[i][2]) == int):
...
def solve_expr(i, n, l):
    ...
    if (type(l[i]) == list and len(l[i]) == 3 and type(l[i][0]) == int and type(l[i][1]) == str and type(
            l[i][2]) == int):
        if (l[i][1] == '+'):
            l[i] = l[i][0] + l[i][2]
        elif (l[i][1] == '-'):
