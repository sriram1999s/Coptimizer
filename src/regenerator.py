...
def solve(start_index, end_index, parse_tree):
    ...
    if (start_index == end_index):
        ...
        if (start_index + 1 < end_index and parse_tree[start_index] in space_list and parse_tree[
            start_index + 1] != ' '):
            output_prg = [parse_tree[start_index], ' ']
        else:
            output_prg = [parse_tree[start_index]]
        ...
        return solve(0, len(parse_tree[start_index]), parse_tree[start_index]) + solve(start_index + 1, end_index,
                                                                                       parse_tree)
